"""
Monitors IRCC, USCIS, and Reddit for the latest immigration news.
...
def fetch_ircc_news() -> list[dict]:
    ...
    try:
        ...
        items = soup.select("li.item article") or soup.select("article.news-item")
        for item in items[:5]:
        ...
        print(f"[monitor] IRCC fetch error: {e}")
    return stories


def fetch_uscis_news() -> list[dict]:
    stories = []
    try:
        resp = httpx.get(USCIS_NEWS_URL, timeout=15, follow_redirects=True)
        soup = BeautifulSoup(resp.text, "html.parser")
        items = soup.select("li.views-row") or soup.select("div.views-row")
        for item in items[:5]:
            title_tag = item.find(["h3", "h2", "a"])
            if not title_tag:
                continue
            ...
            stories.append({"source": "USCIS", "title": title, "url": link})
    except Exception as e:
        print(f"[monitor] USCIS fetch error: {e}")
    return stories
...
def gather_stories() -> list[dict]:
    print("[monitor] Gathering stories...")
    stories = []
    stories.extend(fetch_ircc_news())
    stories.extend(fetch_uscis_news())
